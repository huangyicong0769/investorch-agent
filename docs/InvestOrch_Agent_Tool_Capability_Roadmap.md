# InvestOrch Agent — Tool Capability Roadmap

> 状态：Living Document
>
> 建立日期：2026-08-14
>
> 目的：记录 InvestOrch Agent 在进入多 Agent、数据池和交易系统之前，单 Agent 应具备的通用 Tool 能力、边界、优先级与验收标准，作为后续开发的长期基准。
...
本文档是 `InvestOrch_Agent_Architecture.md` 的工具能力实施路线补充，不替代总架构文档。
...
CNEquity operations:
    human CLI via `investorch data` — available

Agent-managed data lifecycle:
    intentionally not implemented
```

InvestOrch 不为 CNEquity 提供 lifecycle Function Tools、后台任务管理、配置管理或恢复编排。
