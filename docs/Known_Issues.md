...
## Consecutive MiniQMT history downloads can stop reporting completion

**Observed 2026-09-09 with Dongguan Securities MiniQMT 2.0.10.0 and `xtquant==250807.1.2`.** In a minimal same-connection reproduction, the first batch completed in about 11 seconds, but the next immediate batch produced no progress callback or return. This reproduced without the InvestOrch maintenance scheduler; the second batch's 200 daily-cache files were absent both before the request and several minutes later. A full maintenance attempt also reached its two-hour qualification deadline without completing.

Adding a 60-second interval allowed two batches to finish. Restoring zero interval reproduced the wait; a one-second interval then completed four consecutive batches whose 800 cache files were initially absent. Maintenance therefore keeps its internal batching and waits one second between downloads, before the next progress/STOP checkpoint. This is an observed mitigation for the pinned environment, not a confirmed vendor rate limit or a proven explanation of MiniQMT's internal state. Other client builds may behave differently.

The [official API](https://dict.thinktrader.net/nativeApi/xtdata.html#下载历史行情数据) supports synchronous completion and per-instrument progress callbacks. The inspected SDK still has no elapsed-time limit in its completion wait. A [community report](https://www.xuntou.net/forum.php?mod=viewthread&tid=2454) describes the same wait loop but leaves its cause unresolved. Ending a Python client does not establish that its server-side request was cancelled. Download progress remains separate from full-range cache validation: a slow, incomplete, or failed sync cannot advance the global fresh watermark.

## Missing suspension rows can block fresh historical readiness

**Status: observed readiness blocker resolved in the pinned environment (2026-09-09).** The [production qualification](qualification/QMT_Suspension_Gap_Fix_2026-09-09.md) passed all 5,792 supported instruments and two real maintenance runs published READY through September 8. Runtime completeness checks remain in force: unexplained missing dates still prevent fresh historical readiness and watermark promotion.

**Original observation, 2026-09-09.** With the inter-batch interval, all 5,792 supported instruments completed downloading for September 1–8. Independent cache validation passed for 5,779 instruments, including all 577 supported indices, but found missing dates for 13 stocks. The global fresh watermark did not advance. Download completion alone did not establish usable fresh history across the supported scope.

The previous reader accepted an explicit valid raw suspension row but could not resolve a legitimately absent one. The inspected native suspension reference ends on August 31; current instrument status alone does not reconstruct a historical suspension calendar.
...
The initial fresh-window-only `fill_data` probe filled two internal gaps. The later [anchored experiment](qualification/QMT_Suspension_Gap_Probe_2026-09-09.md) recovered all 46 missing dates through provider-filled matrix history with a preceding price anchor and a verified same-exchange index time axis. Production accepts only exact-date rows with explicit suspension status, zero activity, anchored prices and no raw contradiction. `get_bar` and `history_bars(skip_suspended=False)` retain those rows; the default history filter skips them. Only maintenance can ensure missing anchor or index history.

**Remaining evidence limitation.** A real missing-normal-data negative control was not established. The [official documentation](https://dict.thinktrader.net/nativeApi/xtdata.html) describes forward filling but does not independently prove why a row was absent. The pinned observations, normal controls and strict signature support the qualified result; they do not establish universal absence of vendor false positives. Weak evidence and unresolved gaps continue to fail closed. No stock exclusion, manual announcement source or relaxed watermark was used to achieve readiness.
