...
Enabling the SDK's `fill_data` option filled only two internal gaps in the observed sample; it left the other leading, trailing, or entirely empty ranges unresolved. The [official documentation](https://dict.thinktrader.net/nativeApi/xtdata.html) describes forward filling, which does not independently establish why a row was absent. Fresh readiness therefore remains blocked for unexplained gaps. A complete solution needs reliable historical suspension evidence and defined bar values for confirmed suspension days when callers request `skip_suspended=False`; excluding affected stocks or treating every gap as suspension would conceal missing data.
