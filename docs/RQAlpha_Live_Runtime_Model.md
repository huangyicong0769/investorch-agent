...
Deterministic tests cover runtime timing, rejection, strict data mapping, and lifecycle safety. Windows package smoke must actually import the pinned xtquant APIs without connecting. Neither proves real MiniQMT connectivity. Real-machine Level 1 (connect, metadata, subscription, tick, exact-date daily query) and Level 2 (staged Strategy init, READY/RUNNING, truthful status, graceful stop) are the required real-machine acceptance gates for B3 and have passed based on existing Windows/MiniQMT evidence. Level 3 remains pending: a complete trading-day qualification with BEFORE_TRADING, 14:57 execution and rejected order, final daily data, AFTER_TRADING and SETTLEMENT. It is not a B3 merge blocker and must be completed before B4 enables a real Broker. User review and approval are still required before merging.
