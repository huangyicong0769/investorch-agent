...
The companion pins `xtquant==250807.1.2`. Completed historical queries support XSHG/XSHE common stocks (CS) and indexes (INDX), at `1d` frequency. Fresh queries additionally require a verified canonical `.SH`/`.SZ` mapping and matching provider instrument capability. Execution instruments remain CS only. Desired subscriptions are Strategy universe union nonzero Bootstrap holdings. POST_UNIVERSE_CHANGED subscribes the new set before removing the old subscription. Quote callbacks only normalize/cache data; `xtdata.run()` runs in a child liveness thread. A lack of ticks is not a disconnect signal.
...
The data path is `BaseDataSource → FreshDailyDataSource → LiveDataProxy`. Fresh history merges the native raw prefix with the full completed cached tail, orders and deduplicates dates according to the frozen source boundary, applies suspension filtering and a single RQAlpha adjustment pass, then returns the requested count and fields. A missing or invalid fresh range fails closed; it never falls back to stale native data. `skip_suspended=True` retains the native CS positive-volume filtering semantics.

Completed history first reads raw cached bars with `fill_data=False`. Only missing CS dates enter the suspension resolver. It queries provider-filled history together with a same-exchange index whose raw observations must cover every native trading date in the query. Accepted missing rows require the exact date, explicit `suspendFlag=1`, zero volume and amount, finite flat OHLC and previous close matching the preceding provider price. Existing raw observations must remain consistent with the filled result. The index supplies dates, never the stock's price. A missing price anchor, incomplete index axis, contradictory observation or weaker signature leaves the range incomplete. Complete raw ranges and INDX history do not use this resolver.

Strategy reads remain cache-only. After ordinary maintenance downloads, unresolved CS history can trigger a bounded targeted download for the preceding price anchor and required index history, followed by full validation. `get_bar` exposes an accepted completed suspension row; `history_bars(skip_suspended=False)` retains it and the default filtering skips it. These values pass through the same single RQAlpha adjustment step as other historical bars. The [pinned-environment probe](qualification/QMT_Suspension_Gap_Probe_2026-09-09.md) records the provider evidence and the unverified real missing-normal-data negative control; the filled signature is not a claim of universal classification accuracy.
...
A node-wide HistorySyncManager schedules a dedicated spawned maintenance process at 16:30 Asia/Shanghai on native-calendar trading days and performs catch-up after companion startup. It enumerates native XSHG/XSHE CS and INDX candidates within their listing intervals, checks explicit provider identifier and instrument-type support, batches completed raw daily downloads for the supported scope, validates the cache, and reports instrument-count progress. Native metadata remains the candidate authority; provider sector names and name-based aliases do not define that authority. Strategy workers never download bars. Normal daily synchronization adds the completed day; catch-up covers the missing range. Only one maintenance child runs at a time. The ASGI parent does not execute blocking SDK downloads.

Bars remain in MiniQMT's cache. InvestOrch does not maintain a historical bar database or persist factors. On restart, cache coverage is validated again. The global `fresh_through` advances only after the entire requested range passes validation; it does not mean every instrument has a normal bar every day. Combined `available_data_range("1d")` ends at the later of the worker's frozen native cutoff and validated fresh coverage within the declared supported scope. An excluded instrument requiring fresh bars fails with `FRESH_HISTORY_UNSUPPORTED`; native-only queries remain independent of provider support. A cache gap or download failure never removes a supported instrument to obtain READY.

`historical_data` reports READY, SYNCING, or NOT_READY separately from realtime `market_data`, `trading`, control authority, portfolio synchronization, and worker phase. It includes provider, `native_through`, `fresh_through`, `target_through`, and batch progress while syncing. Its scope reports candidate, supported, and excluded counts, exclusion reasons, and the path to the full node-local exclusion metadata report; that report contains no bars or factors. See [Known issues](Known_Issues.md#native-index-identifiers-and-provider-coverage-differ) for observed index coverage limitations. Core `get_live_status` preserves these distinctions: realtime quotes may be ready while completed history is not.
