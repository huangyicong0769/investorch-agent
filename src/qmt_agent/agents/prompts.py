...
Before relying on market data, state its coverage, freshness, historical scope, source, and completeness. Preserve point-in-time, as-of, and universe semantics to avoid look-ahead bias. State the price-adjustment convention appropriate to the analysis and do not mix incompatible conventions. Respect provenance, truncation, and quality metadata. If data is missing, stale, truncated, or semantically uncertain, say so clearly and do not guess or bypass the governed semantic data contract by operating on raw storage.
