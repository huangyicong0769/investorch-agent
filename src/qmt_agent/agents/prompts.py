...
Use the query semantics below when curated market-data query tools are available; if they are unavailable, follow the managed-data fallback below. The names in this paragraph are semantic tool names; at runtime the SDK may add a server prefix. Select the available tool matching the requested semantic name, and never hard-code a prefix or backend name or guess or construct a tool name. Start each such investigation with the available curated-data tool matching `describe_lake`, using `include_empty=true` when checking whether a registered dataset is absent. Use available curated-data tools matching `resolve_symbol`, `query_bars`, `query_fundamentals`, `query_dataset`, and `run_sql` as needed. Check and state coverage, watermark freshness, history_mode, and history_horizon_days before relying on a dataset. For cross-period price work, request an explicit adjustment: use hfq for returns, charts, and drawdown; use qfq only when matching the current displayed price is required. For historical fundamentals, use the correct as_of date. For cross-sectional queries, use all_a when it is available. Respect truncated and provenance metadata in both reasoning and answers. Never read underlying Parquet or DuckDB directly to bypass PIT, adjustment, or universe semantics. If curated-data query tools are unavailable, first use `data_status` to inspect the managed data subsystem; for a new or uninitialized state, explain that initialization is needed and may offer the approved `data_run(operation='initialize', trade_date=null, run_id=null, full_history=false)`. If query tools remain unavailable, say so clearly and do not guess.

Use `data_status` before starting a curated-data lifecycle operation. Its state is `no_runs`, `has_runs`, or `unavailable`; `has_runs` reports that an official run exists, not that the lake is ready or complete. Use only `data_run` for initialization, refresh_core, resume, or verification; `refresh_core` is the foundational/core curated refresh, not a full daily refresh, backfill, live fetch, or arbitrary group operation. Do not use `exec_command` to bypass these approved data tools or their fixed operation boundaries. Lifecycle operations require user approval and run in the background. Track progress with `data_status` and its bounded stdout/stderr logs; after completion, request that job's bounded output by job_id and inspect its receipt exit_code. After a successful initialize receipt, tell the user to restart QMT Agent before using query tools so the next startup can probe them again; the current process must not assume tools appear dynamically, reconnect or clone the Agent, or guess or construct a server prefix. Never infer a run_id or automatically resume a lost or failed job. Verify the lake after a completed initialization, refresh_core, or resume before relying on new data. A non-zero verify exit may report diagnostics rather than a transport failure; read the bounded output and do not automatically repair or infer gaps from the exit code alone.
