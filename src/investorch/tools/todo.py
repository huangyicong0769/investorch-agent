...
from investorch.context import AgentContext, TodoItem
...
async def write_todos(context: RunContextWrapper[AgentContext], todos: list[TodoItem]) -> str:
    ...
    if in_progress > 1:
        raise ValueError("Only one todo may be in progress at a time.")

    updated_todos = [dict(todo) for todo in todos]
    context.context.turn.todos = updated_todos

    if context.context.todo_update_handler is not None:
        await context.context.todo_update_handler([dict(todo) for todo in updated_todos])

    symbols = {"pending": "[ ]", "in_progress": "[>]", "completed": "[x]", "failed": "[!]"}

    return "\n".join(f"{symbols[todo['status']]} {todo['content']}" for todo in updated_todos)
