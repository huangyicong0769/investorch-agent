...
from investorch.storage import SessionRecord, get_session, list_archived_sessions, list_sessions
from investorch.tools import list_background_jobs
...
APPLICATION_VERSION = version("investorch")
...
async def bootstrap(host: Host, broker: Broker) -> dict[str, object]:
    async with host.session_lifecycle_lock:
        ...
        return {
            ...
            "presentation": (
                serialize_session_presentation_state(host.presentation_state.get(initial_session_id))
                if initial_session_id is not None
                else None
            ),
