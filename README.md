...
The independent Windows [InvestOrch QMT companion](packages/investorch-qmt/README.md) provides authenticated MCP controls and REST deployment/fact delivery. It runs each frozen daily stock Strategy in a spawned RQAlpha worker using real MiniQMT/xtdata market data. The trading backend remains unavailable: orders are rejected with `TRADING_BACKEND_NOT_READY` and produce no fills or Ledger mutations. `RUNNING` describes the market runtime, while market and trading health are reported separately. See [remote execution configuration and protocol](docs/QMT_Remote_Execution_Protocol.md).
